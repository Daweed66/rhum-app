...
with tab2:
    ...
    tds = st.tabs(["Mars", "Juin", "Septembre", "Décembre"])
    for i, mois in enumerate(tds):
        with tds[i]:
            d = st.session_state.degustations[mois]
            c1, c2 = st.columns(2)
            c1.metric("Prix Repas", "15 €")
            pb = c2.number_input(f"Prix 5 Bouteilles ({mois})", value=d.get("prix_bouteilles", 0.0), step=5.0, key=f"pb_{mois}")
            ...
            edf = st.data_editor(pd.DataFrame(data_adh), column_config={"Nom": st.column_config.TextColumn(disabled=True)}, hide_index=True, use_container_width=True, key=f"deg_adh_{mois}", height=300)
            ...
            new_inv = c1.text_input("Invité", key=f"ni_{mois}")
            if c2.button("Ajouter", key=f"bi_{mois}") and new_inv:
            ...
            if d["invites"]:
                ...
                inv_ed = st.data_editor(inv_df, hide_index=True, key=f"deg_inv_{mois}")
...
with tab4:
    ...
    for m, d in st.session_state.mois_data.items():
        if d["nom_bouteille"]:
            ...
            # On considère que 2 samples partent en rhumothèque d'office
            ...
            if restant > 0:
                ...
                tot_val += val
                rows.append({"Mois": m, "Bouteille": d["nom_bouteille"], "Vendus": vendus, "Rhumothèque": preleves, "Restant": restant, "Valeur": val})
...
with tab5:
    ...
    st.info(f"ℹ️ Chaque mois, {RHUMOTHEQUE_PRELEVEMENT} samples (6cl) sont réservés automatiquement ici dès qu'une bouteille est créée.")
    ...
    for i, (mois, data) in enumerate(st.session_state.rhumotheque.items()):
        if not data.get("nom"): continue # Ignorer vides
        
        with cols[i % 3]:
            with st.container(border=True):
                st.markdown(f"**{mois}** : {data['nom']}")
                st.caption(f"Valeur : {data['valeur']:.2f} € (2 samples)")
    ...
    c2.metric("Valeur Totale Rhumothèque", f"{tot_val_rhumo:.2f} €", help="Somme des prix des samples stockés")
